...
const filterFields = [
  ...
  { field: "criteria", operator: "___name__lookalike", value: null },
];

export function useSearchScoringCategories() {
  ...
  const {
    isPending,
    data: { data: scoringCategories, count } = {},
    error,
  } = useQuery({
    queryKey: scoringCategoriesKeys.list(filters, sortBy, page, size),
    queryFn: () => getSearchScoringCategories({ filters, sortBy, page, size }),
    placeholderData: keepPreviousData,
  });

  return { isPending, error, scoringCategories, count };
}
